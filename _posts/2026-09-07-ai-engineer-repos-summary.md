...
Shraddha Bharuka(`@BharukaShraddha`)가 북마크해 두라는 [AI·ML·LLM·에이전트 학습용 GitHub 레포 10개](https://x.com/BharukaShraddha/status/2096296838811074949)를 **정보용 라이브러리**로 옮겨 둔 요약이다. 튜토리얼을 대신하지 않고, 어디에 무엇이 있는지 한눈에 보기 위한 목록이다.
...
원문 취지는 단순하다. AI를 배울 때 튜토리얼만 떠돌지 말고, **실제로 북마크할 만한 레포**를 단계별로 잡아 두라는 것. 전부 저장해 두고 잊기보다, 지금 위치에 맞는 **하나**를 골라 무언가를 만든 뒤 다음으로 가라고 한다.
...
이 글은 추천 순위를 새로 매기지 않는다. 원문이 고른 10개를 **참조용 선반**에 올려 둔 것이다. 필요할 때 링크만 찾아가면 된다.
